...
#	temp_path = "tempfile.txt"
	...
	header = None
	temp_idx = 0
	while os.path.exists("{}_{}_temp.txt".format(os.path.splitext(os.path.basename(args.vcf_path))[0], temp_idx)):
		temp_idx += 1
	while last_pos < region_end or args.bed is not None:
		temp_path = "{}_{}_temp.txt".format(os.path.splitext(os.path.basename(args.vcf_path))[0], temp_idx)
...
#			last_pos = args.query_size
			...
			if args.bed is not None:
				bcft_cmd = "{}*-R*{}".format(bcft_cmd, args.bed)
			else:
				region = "{}:{}-{}".format(region_name, last_pos, min(last_pos + args.query_size, region_end))
		...
		for bad_pos in bad_pos_list:
			del gt_dict[bad_pos]
		bad_pos_list = []
		while len(gt_dict) > args.chunk_size or (last_pos >= region_end and len(gt_dict) > 0) or (args.bed is not None and len(gt_dict) > 0):
			fasta_dict = {sample_id: "" for sample_id in sample_ids}
			if args.bed is not None:
				pos_list = sorted(gt_dict.keys())
				pos_map_fname = os.path.join(args.output, os.path.splitext(os.path.basename(args.bed))[0] + "_positions.txt")
				fasta_fname = os.path.join(args.output, os.path.splitext(os.path.basename(args.bed))[0] + ".fasta")
			else:
...
if __name__ == '__main__':
	...
	parser.add_argument("--bed", help="*.bed file to get coordinate ranges from.", type=str, default=None)
